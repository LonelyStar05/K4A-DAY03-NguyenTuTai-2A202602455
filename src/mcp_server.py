...
class MCPAcademicServer:
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. Call dispatch_tool_call to get JSON string result
        json_result_str = dispatch_tool_call(tool_name, arguments)
        
        # 2. Parse JSON string to Python Dictionary
        try:
            content = json.loads(json_result_str)
        except json.JSONDecodeError:
            # Fallback if result is not valid JSON
            content = {"error": "Invalid JSON response from tool", "raw": json_result_str}
            
        # 3. Wrap response in MCP JSON-RPC 2.0 format
        return {
            "jsonrpc": "2.0",
